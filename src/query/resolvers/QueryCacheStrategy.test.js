...
describe('query cache', () => {
  test('onUnobservedStrategy KEEP_UPDATING', async () => {
    ...
    expect(data).toEqual({ __typename: "Person", id: "person2", name: "John" });
  ...
  test('onUnobservedStrategy PAUSE_UPDATING', async () => {
    ...
    expect(data).toEqual({ id: 'person2', __typename: 'Person', name: 'John' });
  ...
  test('store updates', async () => {
    ...
    expect(data1).toEqual({ id: 'person2', __typename: 'Person', name: 'John' });

    const data2 = await query2.query({ foo: 1 });

    expect(data1).toEqual({ id: 'person2', __typename: 'Person', name: 'John' });
    expect(data2).toEqual({ id: 'person2', __typename: 'Person', name: 'James' });

    const data3 = await query1.query({ foo: 1 });

    expect(data3).toEqual({ id: 'person2', __typename: 'Person', name: 'James' });
  });
  ...
  test('store updates unrelated', async () => {
    ...
    expect(data1).toEqual({ id: 'person2', __typename: 'Person', name: 'John' });
    expect(data2).toEqual({ id: 'person3', __typename: 'Person', name: 'James' });

    const data3 = await query1.query({ foo: 1 });

    expect(data3).toEqual({ id: 'person2', __typename: 'Person', name: 'John' });
  });
  ...
  test('custom onStoreUpdate: update', async () => {
    ...
    query1.setOnStoreUpdate((update, _variables, match) => {
      if (match(update, { type: UpdateType.UPDATE_PROP, entity: { id: 'person2' } })) {
        return cache => ({ ...cache, name: update.entity.name });
      }
    });
